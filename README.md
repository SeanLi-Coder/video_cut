...
一个面向 macOS 的本地视频处理工具，提供“剪辑视频”“逐帧截图”和“永久旋转”三种模式。双击 `start.command` 就能用；视频不会上传到网络，原文件也不会被修改。
...
剪辑模式下点击“确定并导出”；逐帧截图模式下点击“确定并截图”；永久旋转模式下点击“确定并旋转”。完成后可以直接点击“在 Finder 中显示”。三种操作都会创建新内容，原视频始终保留不变。
...
- 预览、最终剪辑、逐帧截图和永久旋转都由本机 FFmpeg 完成。
