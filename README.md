...
页面会显示当前阶段、百分比、已下载容量、下载速度和耗时。切换回“视频处理”不会中断；刷新页面后也会自动找回正在运行的任务。取消后已下载部分会保留，下次点击“继续下载”会断点续传。全部完成并通过 SHA-256 校验后，AI 超清可以直接加载模型。
...
模型管理页支持查看实时百分比、下载容量、速度和耗时。中途取消会停止下载或安装进程；已完整下载的模型会保留，`.download` 临时权重也会保留以便下次续传。AI 视频任务中途取消时会停止整个 AI/FFmpeg 进程组并删除未完成成片。以后双击 `start.command` 不会重复安装或下载。
